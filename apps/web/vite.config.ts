...
const devServerPort = toNumber(
  process.env.VITE_DEV_SERVER_PORT ?? process.env.PORT,
...
const wsProxyTarget =
  ...
  apiProxyTarget.replace(/^http(s?):/, (_, secure) => (secure ? "wss:" : "ws:"));
const watchIntervalMs = toNumber(
