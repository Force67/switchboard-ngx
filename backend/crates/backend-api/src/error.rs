...
impl From<switchboard_database::ChatError> for GatewayError {
    fn from(error: switchboard_database::ChatError) -> Self {
        match error {
            switchboard_database::ChatError::ChatNotFound => {
                GatewayError::NotFound("Chat not found".to_string())
            }
            switchboard_database::ChatError::MessageNotFound => {
                GatewayError::NotFound("Message not found".to_string())
            }
            switchboard_database::ChatError::MemberNotFound => {
                GatewayError::NotFound("Member not found".to_string())
            }
            switchboard_database::ChatError::InviteNotFound => {
                GatewayError::NotFound("Invite not found".to_string())
            }
            switchboard_database::ChatError::AttachmentNotFound => {
                GatewayError::NotFound("Attachment not found".to_string())
            }
            switchboard_database::ChatError::MemberAlreadyExists => {
                GatewayError::InvalidRequest("Member already exists".to_string())
            }
            switchboard_database::ChatError::InviteAlreadyUsed => {
                GatewayError::InvalidRequest("Invite has already been used".to_string())
            }
            switchboard_database::ChatError::InviteExpired => {
                GatewayError::InvalidRequest("Invite has expired".to_string())
            }
            switchboard_database::ChatError::AccessDenied => {
                GatewayError::AuthorizationFailed("Access denied".to_string())
            }
            switchboard_database::ChatError::Unauthorized => {
                GatewayError::AuthenticationFailed("Unauthorized".to_string())
            }
            switchboard_database::ChatError::ChatArchived => {
                GatewayError::AuthorizationFailed("Chat is archived".to_string())
            }
            switchboard_database::ChatError::ServiceUnavailable(msg) => {
                GatewayError::ServiceError(msg)
            }
            switchboard_database::ChatError::DatabaseError(msg) => GatewayError::DatabaseError(msg),
        }
    }
}
