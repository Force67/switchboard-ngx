...
impl ApiClient {
    ...
    async fn list_chats(&self, token: &str) -> Result<Vec<Chat>> {
        ...
        println!("=� Found {} chats", chats_response.chats.len().to_string().yellow());
    ...
    async fn get_messages(&self, token: &str, chat_id: &str) -> Result<Vec<Message>> {
        println!("=� Fetching messages for chat: {}", chat_id);
        ...
        println!("=� Found {} messages", messages_response.messages.len().to_string().yellow());
...
async fn test_chat_management(api_client: &ApiClient, user_token: &str, count: u32) -> Result<()> {
    ...
    println!("=� User currently has {} chats", existing_chats.len());
...
async fn test_complete_workflow(api_client: &ApiClient, user_count: u32, chats_per_user: u32, messages_per_chat: u32) -> Result<()> {
    ...
    // Step 1: Create users
    println!("\n=� Step 1: Creating {} users", user_count);
    let mut users = Vec::new();
    ...
    println!("\n=� Step 3: Sending {} messages per chat", messages_per_chat);
    ...
    for (token, chat) in &all_chats {
        match api_client.get_messages(token, &chat.public_id).await {
            Ok(messages) => {
                ...
                println!("=� Chat '{}' has {} messages", chat.title, messages.len());
            }
            Err(e) => {
