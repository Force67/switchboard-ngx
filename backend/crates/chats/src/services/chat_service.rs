...
use switchboard_database::{
    Chat, ChatRepository, ChatResult, CreateChatRequest, MemberRepository, MemberRole,
    UpdateChatRequest,
};
...
impl ChatService {
    ...
    pub async fn create(&self, request: &CreateChatRequest) -> ChatResult<Chat> {
        let user_id = request.created_by.parse::<i64>().map_err(|_| {
    ...
    pub async fn update(&self, chat_id: i64, request: &UpdateChatRequest) -> ChatResult<Chat> {
        ...
        let user_id = chat.created_by.parse::<i64>().map_err(|_| {
            switchboard_database::ChatError::DatabaseError("Invalid created_by user ID".to_string())
        })?;
    ...
    pub async fn update_chat(
        ...
        let chat = self
            .chat_repository
            .update(chat_id, user_id, &request)
            .await?;
    ...
    pub async fn check_membership(&self, chat_id: i64, user_id: i64) -> ChatResult<()> {
        ...
        let member = self
            .member_repository
            .find_by_user_and_chat(user_id, &chat.public_id)
            .await?;
        if member.is_none() {
