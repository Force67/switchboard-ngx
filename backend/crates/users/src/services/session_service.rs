...
use switchboard_database::{
    AuthError, AuthResult, AuthSession, CreateSessionRequest, SessionRepository,
};
...
impl SessionService {
    ...
    pub async fn create_session(&self, request: CreateSessionRequest) -> AuthResult<AuthSession> {
        ...
        let active_sessions = self
            .session_repository
            .count_active_sessions(request.user_id)
            .await
        ...
        if active_sessions as u32 >= MAX_SESSIONS_PER_USER {
            ...
            log::warn!(
                "User {} exceeded max sessions, cleaned up oldest session",
                request.user_id
            );
        }
    ...
    pub async fn validate_session(&self, token: &str) -> AuthResult<AuthSession> {
        ...
        self.session_repository
            .update_last_used(&session.token)
            .await
            .map_err(|e| AuthError::DatabaseError(e.to_string()))?;
    ...
    pub async fn create_dev_token(&self) -> AuthResult<(AuthSession, switchboard_database::User)> {
        use switchboard_database::{CreateUserRequest, User, UserRepository, UserRole};

        // Create or get dev user
        let user_repo = UserRepository::new(self.pool.clone());
        let dev_user = match user_repo
            .find_by_id(1i64)
            .await
            .map_err(|e| AuthError::DatabaseError(e.to_string()))?
        {
